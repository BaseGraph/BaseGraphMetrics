...
std::vector<double>
getJaccardReciprocities(const DirectedGraph &graph,
    ...
    if (reciprocities.size() != graph.getSize() ||
        ...
        throw logic_error("The reciprocities and the in degrees must have the "
                          "size of the graph");

    vector<double> jaccardReciprocities(reciprocities.begin(),
                                        reciprocities.end());

    for (const VertexIndex &vertex : graph)
        jaccardReciprocities[vertex] /= inDegrees[vertex] +
                                        graph.getOutDegreeOf(vertex) -
...
std::vector<double>
getReciprocityRatios(const DirectedGraph &graph,
                     const std::vector<size_t> reciprocities,
                     const std::vector<size_t> inDegrees) {
    if (reciprocities.size() != graph.getSize() ||
        inDegrees.size() != graph.getSize())
    ...
    vector<double> reciprocityRatios(reciprocities.begin(),
                                     reciprocities.end());

    for (const VertexIndex &vertex : graph)
        reciprocityRatios[vertex] *=
            (double)2 / (inDegrees[vertex] + graph.getOutDegreeOf(vertex));

    return reciprocityRatios;
}

vector<double>
getUndirectedLocalClusteringCoefficients(const DirectedGraph &graph) {
    auto inEdges = graph.getInEdges();
    return getUndirectedLocalClusteringCoefficients(
        graph, findAllDirectedTriangles(graph, inEdges), inEdges);
}

vector<double>
getUndirectedLocalClusteringCoefficients(const DirectedGraph &graph,
                                         const AdjacencyLists &inEdges) {
    return getUndirectedLocalClusteringCoefficients(
        graph, findAllDirectedTriangles(graph, inEdges), inEdges);
}

vector<double> getUndirectedLocalClusteringCoefficients(
    const DirectedGraph &graph, const list<array<VertexIndex, 3>> &triangles,
    const AdjacencyLists &inEdges) {
    if (inEdges.size() != graph.getSize())
        throw logic_error("The inEdges vector must be the size of the graph");
    ...
    for (const VertexIndex &vertex : graph) {
        undirectedDegree =
            getUnionOfLists(graph.getOutEdgesOf(vertex), inEdges[vertex])
                .size();
...
list<array<VertexIndex, 3>>
findAllDirectedTriangles(const DirectedGraph &graph) {
...
findAllDirectedTriangles(const DirectedGraph &graph,
    ...
    for (const VertexIndex &vertex1 : graph)
        undirectedEdges[vertex1] =
            getUnionOfLists(graph.getOutEdgesOf(vertex1), inEdges[vertex1]);
...
map<string, size_t>
getTriangleSpectrum(const DirectedGraph &graph,
                    const list<array<VertexIndex, 3>> &triangles) {
