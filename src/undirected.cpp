...
size_t countTrianglesAroundVertex(const UndirectedGraph &graph,
                                  VertexIndex vertex1) {
    size_t triangleNumber = 0;
    const auto &vertexNeighbourhood = graph.getNeighboursOf(vertex1);
    ...
    for (const auto &vertex2 : vertexNeighbourhood)
        for (const auto &vertex3 : graph.getNeighboursOf(vertex2))
...
static size_t countTrianglesAroundVertex(const UndirectedGraph &graph,
                                         VertexIndex vertex1,
                                         std::vector<bool> &adjacencyFlags) {
    for (const auto &neighbour : graph.getNeighboursOf(vertex1))
        adjacencyFlags[neighbour] = 1;

    size_t triangleNumber = 0;
    for (const auto &vertex2 : graph.getNeighboursOf(vertex1))
        for (const auto &vertex3 : graph.getNeighboursOf(vertex2))
...
list<array<VertexIndex, 3>> findAllTriangles(const UndirectedGraph &graph) {
    ...
    for (auto &vertex1 : graph) {
        const auto &vertex1Neighbours = graph.getNeighboursOf(vertex1);
        ...
        for (auto &vertex2 : vertex1Neighbours)
            if (vertex1 < vertex2)
                for (auto &vertex3 : graph.getNeighboursOf(vertex2))
                    if (vertex2 < vertex3 &&
...
double getGlobalClusteringCoefficient(const UndirectedGraph &graph) {
    ...
    for (VertexIndex &vertex : graph) {
        size_t vertexDegree = graph.getDegreeOf(vertex);
...
double
getGlobalClusteringCoefficient(const UndirectedGraph &graph,
                               const vector<size_t> &vertexTriangleNumbers) {
    size_t vertexDegree;
...
vector<double> getRedundancy(const UndirectedGraph &graph) {
    ...
    for (VertexIndex vertex : graph)
        localClusteringCoefficients[vertex] *=
            (double)(graph.getDegreeOf(vertex) - 1);
...
getClusteringSpectrum(const UndirectedGraph &graph) {
...
getKShellsAndOnionLayers(const UndirectedGraph &graph) {
...
list<size_t> getNeighbourhoodDegreesOfVertex(const UndirectedGraph &graph,
                                             VertexIndex vertex) {
...
vector<double> getNeighbourDegreeSpectrum(const UndirectedGraph &graph,
                                          bool normalized) {
    vector<double> degreeSpectrum(graph.getSize());

    for (VertexIndex &vertex : graph)
        degreeSpectrum[vertex] =
            getAverage(getNeighbourhoodDegreesOfVertex(graph, vertex));
...
unordered_map<size_t, list<double>>
getOnionSpectrum(const UndirectedGraph &graph) {
...
unordered_map<size_t, list<double>>
getOnionSpectrum(const UndirectedGraph &graph, const vector<size_t> &kshells,
...
double getModularity(const UndirectedGraph &graph,
                     const vector<size_t> &vertexCommunities) {
    if (graph.getSize() == 0)
...
static size_t count4Cliques(const UndirectedGraph &graph, std::vector<int> &X,
...
static size_t countCycles(const UndirectedGraph &graph, std::vector<int> &X,
...
countMotifs(const UndirectedGraph &graph, size_t maxMotifSize) {
    if (maxMotifSize > 4)
        fprintf(stderr,
    ...
    for (const auto &edge : graph.edges()) {
        ...
        N_II1 += edgeNumber - graph.getDegreeOf(u, false) -
                 graph.getDegreeOf(v, false) + 1;
